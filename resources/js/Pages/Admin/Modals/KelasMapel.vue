						...
						<v-row class="mt-2">
							<v-col cols="12" sm="6">
								<v-card>
									...
									<v-card-text>
										<v-list >
											<v-list-item-group v-model="selected" multiple dense>
							...
							<v-col cols="12" sm="6">
								<v-card>
									...
									<v-card-text>
										<v-list>
											<v-list-item-group v-model="selectedX" multiple dense>
	...
	export default {
		...
		methods: {
			...
			kd(mapel){
				...
				axios({
					method: 'post',
					url: '/admin/kd?kelas='+kelas+'&mapel='+mapel
				}).then( response => {
			...
			init(kelas) {
				...
				axios({
					method: 'post',
					url: '/admin/mapel/tanpakelas?kelas_id='+kelas.id
				}).then( response => {
					let mapels = response.data.mapels
					// mapels.forEach( mapel => {
			...
			masukkan() {
				...
				axios({
					method: 'post',
					url: '/admin/kelas/'+this.dialog.kelas.id+'/mapel/attach',
			...
			keluarkan() {
				axios({
					method: 'post',
					url: '/admin/kelas/'+this.dialog.kelas.id+'/mapel/detach',
					data: { mapels: this.selected }
				}).then( res => {
					this.init(this.dialog.kelas)
